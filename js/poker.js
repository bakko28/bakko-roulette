const cards = [{items: [
    // aces
    { id: 'As', dignity: 10,  suit: 'spades', used: false, img: '../img/cards/aces/as.png'},
    { id: 'Ah', dignity: 10,  suit: 'hearts', used: false, img: '../img/cards/aces/ah.png' },
    { id: 'Ad', dignity: 10,  suit: 'diamonds', used: false, img: '../img/cards/aces/ad.png' },
    { id: 'Ac', dignity: 10,  suit: 'clubs', used: false, img: '../img/cards/aces/ac.png' },
    // kings
...
function genRandomCard() {
    ...
    if (card.used != true) {
        ...
        console.log('Карта уже выбрана');
        return genRandomCard();
    }
}
...
function startGame() {
    return new Promise((resolve, reject) => {
        const newGameId = games.length ? games[games.length - 1].game_id + 1 : 1;
        games.push({ id: newGameId, status: 'goes' });

        for (let i = 0; i < 13; i++) {
            selectedCards.push(genRandomCard());
        }
...
function checkStatus() {
    ...
    let statusGame = lastGame.status

    if (statusGame != 'goes') {
        cards[0].items.forEach(card => {
            card.used = false;
        });
        game();
    } else {
